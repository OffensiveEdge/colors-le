# Colors-LE Testing Guidelines

This document outlines testing practices and policies for Colors-LE development.
...
```bash
bun x vitest run src/extraction/formats/css.test.ts
```
...
- **Format tests** - Test CSS, HTML, JSON, etc. extraction
...
```typescript
// ✅ Good
it('should extract hex colors from CSS files', () => {
  // ...
});
...
it('should extract hex and rgb colors', () => { /* ... */ });
```
